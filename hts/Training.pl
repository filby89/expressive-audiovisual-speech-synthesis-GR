...
$scp{'gen'} = "$datdir/scp/$spkr/$emotion/$dataversion/test.scp";

# model list files
$lst{'mon'} = "$datdir/lists/$spkr/$emotion/mono.list";
$lst{'ful'} = "$datdir/lists/$spkr/$emotion/full.list";
$lst{'all'} = "$datdir/lists/$spkr/$emotion/full_all.list";

# master label files
$mlf{'mon'} = "$datdir/labels/$spkr/$emotion/mono.mlf";
$mlf{'ful'} = "$datdir/labels/$spkr/$emotion/full.mlf";

# configuration variable files
$cfg{'trn'} = "$synthesisdir/configs/qst${qnum}/ver${ver}/trn.cnf";
$cfg{'nvf'} = "$synthesisdir/configs/qst${qnum}/ver${ver}/nvf.cnf";
$cfg{'syn'} = "$synthesisdir/configs/qst${qnum}/ver${ver}/syn.cnf";
$cfg{'apg'} = "$synthesisdir/configs/qst${qnum}/ver${ver}/apg.cnf";
$cfg{'stc'} = "$synthesisdir/configs/qst${qnum}/ver${ver}/stc.cnf";
foreach $type (@cmp) {
   $cfg{$type} = "$synthesisdir/configs/qst${qnum}/ver${ver}/${type}.cnf";
}
foreach $type (@dur) {
   $cfg{$type} = "$synthesisdir/configs/qst${qnum}/ver${ver}/${type}.cnf";
}
...
$prtfile{'cmp'} = "$synthesisdir/proto/qst${qnum}/ver${ver}/state-${nState}_stream-$nstream{'cmp'}{'total'}";
foreach $type (@cmp) {
...
# model files
foreach $set (@SET) {
   $model{$set}   = "$synthesisdir/models/qst${qnum}/ver${ver}/${set}";
   ...
   $reclmmf_adp{$set} = "$model{$set}/re_clustered_adp.mmf";
...
# statistics files
foreach $set (@SET) {
   $stats{$set} = "$synthesisdir/stats/qst${qnum}/ver${ver}/${set}.stats";
}

# model edit files
foreach $set (@SET) {
   $hed{$set} = "$synthesisdir/edfiles/qst${qnum}/ver${ver}/${set}";
   $lvf{$set} = "$hed{$set}/lvf.hed";
   $m2f{$set} = "$hed{$set}/m2f.hed";
   $mku{$set} = "$hed{$set}/mku.hed";
   $mkadp{$set} = "$hed{$set}/mkadp.hed";
   $mkintl{$set} = "$hed{$set}/mkintl.hed";
...
# decision tree files
foreach $set (@SET) {
   $trd{$set} = "${synthesisdir}/trees/qst${qnum}/ver${ver}/${set}";
   foreach $type ( @{ $ref{$set} } ) {
...
$voice = "$synthesisdir/voices/qst${qnum}/ver${ver}";
foreach $set (@SET) {
   foreach $type ( @{ $ref{$set} } ) {
...
$gvdir         = "$synthesisdir/gv/qst${qnum}/ver${ver}";
$gvfaldir      = "$gvdir/fal";
$gvfaldir_state      = "$gvdir/fal_state";
$gvdatdir      = "$gvdir/dat";
$gvlabdir      = "$gvdir/lab";
$scp{'gv'}     = "$gvdir/gv.scp";
$mlf{'gv'}     = "$gvdir/gv.mlf";
...
$clusmmf_adp{'gv'} = "$gvdir/clustered_adp.mmf";
$tiedlst{'gv'} = "$gvdir/tiedlist";
$mku{'gv'}     = "$gvdir/mku.hed";
$mkadp{'gv'}     = "$gvdir/mkadp.hed";
$mkintl{'gv'}     = "$gvdir/mkintl.hed";
...
$mspfdir     = "$synthesisdir/mspf/qst${qnum}/ver${ver}";
...
$qconf = "$prjdir/data/questions/questions.conf";
...
$HHEdNonBinary{'trn'}   = "$HHED      -A -C $cfg{'trn'} -D -T 1 -p -i ";
$HSMMAlign     = "$HSMMALIGN -A    -C $cfg{'trn'} -D -T 1 -S $scp{'trn'} -I $mlf{'mon'} -t $beam -w 1.0 ";

$HSMMAlignState     = "$HSMMALIGN -A    -C $cfg{'trn'} -D -T 1 -S $scp{'trn'} -I $mlf{'ful'} -t $beam -w 1.0 ";
...
if ($MKEMV) {
   ...
   foreach $dir ( 'models', 'stats', 'edfiles', 'trees', 'gv', 'mspf', 'voices', 'gen', 'proto', 'configs' ) {
      mkdir "$synthesisdir/$dir",                      0755;
      mkdir "$synthesisdir/$dir/qst${qnum}",           0755;
      mkdir "$synthesisdir/$dir/qst${qnum}/ver${ver}", 0755;
   }
   foreach $set (@SET) {
...
if ($FALGN) {
   ...
   if ( ( $useGV && $nosilgv && @slnt > 0 ) || ($useMSPF) ) {
     ...
     shell("$HSMMAlign -H $monommf{'cmp'} -N $monommf{'dur'} -m $gvfaldir $lst{'mon'} $lst{'mon'}");
      # shell("$HSMMAlignState -f -H $reclmmf{'cmp'} -N $reclmmf{'dur'} -m $gvfaldir $lst{'ful'} $lst{'ful'}");
...
#       mkdir "$gvfaldir_state", 0755;

#       # forced alignment
#       shell("$HSMMAlign -f -H $reclmmf{'cmp'} -N $reclmmf{'dur'} -m $gvfaldir_state $lst{'ful'} $lst{'ful'}");
...
# # HSMMAlign state level (forced alignment)
# if ($FALGNS) {
#    print_time("forced alignment");
...
#       # forced alignment
#       shell("$HSMMAlignState -f -H $reclmmf{'cmp'} -N $reclmmf{'dur'} -m $gvfaldir_state $lst{'ful'} $lst{'ful'}");
...
if ($MKUN1) {
   ...
   foreach $set (@SET) {
      ...
      shell("$HHEd{'trn'} -H $reclmmf{$set} -w $rclammf{$set}.$mix $mku{$set} $lst{'ful'}");
      # shell("$HHEdNonBinary{'trn'} -H $reclmmf{$set} -w $rclammf{$set}.$mix $mku{$set} $lst{'ful'}");
   }
}
...
#    $mix = '1mix';
#    foreach $set (@SET) {
#       make_edfile_mk_intl($set);
#       # shell("$HHEd{'trn'} -H $reclmmf{$set} -w $rclammf{$set}.$mix $mku{$set} $lst{'ful'}");
#       shell("$HHEdNonBinary{'trn'} -H $reclmmf{$set} -w $rclammf{$set}.$mix $mkintl{$set} $lst{'ful'}");
#    }
# }
...
if ($PGEN1) {
   ...
   $mix = '1mix';
   $dir = "${synthesisdir}/gen/qst${qnum}/ver${ver}/$mix/$pgtype";
   mkdir "${synthesisdir}/gen/qst${qnum}/ver${ver}/$mix", 0755;
   mkdir $dir, 0755;

   # generate parameter
   # shell("$HMGenS -S $scp{'intl'} -c $pgtype -H /home/filby/av_synthesis/scripts/0.100000_neutral_0.900000_happy.mmf.1mix -N $rclammf{'dur'}.$mix -M $dir $tiedlst{'cmp'} $tiedlst{'dur'}");
...
########################### THIS IS ONLY TO BE USED FOR MERLIN OUTPUT ##############################
# SPTK (synthesizing waveforms (1mix))
if ($WGENMERLIN) {
   gen_wave("/home/kinect/filby/av_synthesis/merlin/egs/greek/s1/experiments/cvsp_greek_voices_dt_happy/acoustic_model/gen/DNN_TANH_TANH_TANH_TANH_TANH_TANH_LINEAR__mgc_lf0_vuv_bap_shape_texture_1_744_1342_373_6_1024_1024");
      # gen_video("/home/filby/workspace/phd/audiovisual_speech_synthesis/merlin/egs/greek/s1/experiments/cvsp_ip/test_synth/wav");

}
########################### THIS IS ONLY TO BE USED FOR MERLIN OUTPUT END ##############################
...
sub make_data_gv {
   ...
   while (<SCP>) {
      ...
      foreach $type (@cmp) {
         ...
         #          if ($type eq "lf0")  {
         #    shell("$BCUT -s $start -e $end -l $ordr{$type} +f $datdir/$type/$spkr/$emotion/$base.$type >> $gvdatdir/tmp.$type");    
         # }
        if ($type eq "mgc" || $type eq "bap" || $type eq "lf0"){ 
                        shell("$BCUT -s $start -e $end -l $ordr{$type} +f $datdir/$SPFEATURES_DIR/$type/$base.$type >> $gvdatdir/tmp.$type");      
         }
        elsif ($type eq "vfs") {
                 shell("$BCUT -s $start -e $end -l $ordr{$type} +f $VIDFEATURES_DIR/$base.mfc >> $gvdatdir/tmp.$type");    #changed 
                         }
        elsif ($type eq "shape") {
                 shell("$BCUT -s $start -e $end -l $ordr{$type} +f $SHAPE_DIR/$base.mfc >> $gvdatdir/tmp.$type");    #changed
                         }    
        elsif ($type eq "texture") {
                 shell("$BCUT -s $start -e $end -l $ordr{$type} +f $TEXTURE_DIR/$base.mfc >> $gvdatdir/tmp.$type");    #changed
                         }                     
               }
      }
            close(F);
      ...
      if ( length($i) > 0 ) {
         ...
         if ($cdgv) {
            open( LAB, "$datdir/labels/$spkr/$emotion/full/$base.lab" ) || die "Cannot open $!";
            $str = <LAB>;
            close(LAB);
            chomp($str);
...
sub make_config {
   ...
   print CONF "USEGVPST = \"IntVec $nPdfStreams{'cmp'}";    # PdfStream structure
   foreach $type (@cmp) {
      print CONF " $usegvpst{$type}";
   }
   print CONF "\"\n";
...
#############################
   ...
   # print CONF "GVMODELMMF = $clsammf_intl{'gv'}\n"; # TODO
...
sub gen_wave($) {
   ...
   foreach $file (@FILE) {
      ...
      if ( !$usestraight && -s $file && -s $lf0 ) {
         ...
         if ( $gm == 0 ) {
            shell( "$MGC2SP -a $fw -g $gm -m " . ( $ordr{'mgc'} - 1 ) . " -l $fftlengthstr -o 2 $mgc > $gendir/$base.sp" );
         }
         else {
            shell( "$MGC2SP -a $fw -c $gm -m " . ( $ordr{'mgc'} - 1 ) . " -l $fftlengthstr -o 2 $mgc > $gendir/$base.sp" );
         }

         # convert band-aperiodicity to aperiodicity
         shell( "$MGC2SP -a $fw -g 0 -m " . ( $ordr{'bap'} - 1 ) . " -l $fftlengthstr -o 0 $bap > $gendir/$base.ap" );
         ...
         printf SYN "sp = fread(fid1,[%d, %d],'float');\n",  1025, $T; ## 1025 -> 512 for straight trial
...
#         printf SYN "sp = sp*(" . ( 1024.0 / ( 2200.0 * 32768.0 ) ) . ");\n";    # normalization for STRAIGHT (sdev of amplitude is set to 1024)         printf SYN "wavwrite( sy, %d, '%s');\n\n", $sr, "$gendir/$base.wav";
...
sub postfiltering_mspf($$$) {
   ...
   $gentype = $gendir;
   $gentype =~ s/$synthesisdir\/gen\/qst$qnum\/ver$ver\/+/gen\//g;
   ...
   while (<SCP>) {
      ...
      foreach $type ('mgc') {
         foreach $mspftype ( 'nat', $gentype ) {
            ...
            if   ( $mspftype eq 'nat' ) { $orgdir = "$datdir/$SPFEATURES_DIR/$type"; }
...
sub gen_video($) {
   my ($gendir) = @_;
   my ( $line, @FILE, $file, $base, $i );   


   $line  = `ls $gendir/*.shape`;
   @FILE  = split( '\n', $line );

   open( SYN, ">$gendir/vidsynthesis_$ver.m" ) || die "Cannot open $!";
   ...
   # -- for debugging purposes -- #
      # shape
      $base = `basename $file .shape`;
      chomp($base);
      $i = 4 * $ordr{'shape'};
      # print "Bytes per frame $i\n\n";
      shell("$PERL ${datdir}/scripts/addhtkheader.pl $sr $fs $i 9 $gendir/$base.shape > $gendir/$base.shape_h");

#      $base = `basename $file .texture`;
#      chomp($base);
      $i = 4 * $ordr{'texture'};
      # print "Bytes per frame $i\n\n";
      shell("$PERL ${datdir}/scripts/addhtkheader.pl $sr $fs $i 9 $gendir/$base.texture > $gendir/$base.texture_h");
}

   printf SYN "cd $aam_tools_dir;\n";
   printf SYN "addpath mex;\n";
   printf SYN "addpath utils\n";
   printf SYN "params.num_sh = %d;\n", "$num_sh";
   printf SYN "params.num_te = %d;\n", "$num_te";
   printf SYN "params.fps = %f;\n", "$fps_synth";
   printf SYN "figure('Visible','Off')\n";
   printf SYN "parpool($num_cores)\n";

   $line  = `ls $gendir/*.shape_h`;
   @FILE  = split( '\n', $line );

   foreach $file (@FILE){  
      $base = `basename $file .shape_h`;
      chomp($base);
      printf SYN "fprintf(1,'\\nSynthesizing images for %s\\n');\n\n", "$base.mkv";
      printf SYN "convert_to_vid('%s', '%s', '%s', '%s', '%s', '%s', params);\n", "$full_model", "$gendir/$base.shape_h", "$gendir/$base.texture_h", "$gendir/$base.wav", "$gendir/$base.mkv", "$gendir";
   }
   ...
   shell("$MATLABV < $gendir/vidsynthesis_$ver.m");
   print "done\n";

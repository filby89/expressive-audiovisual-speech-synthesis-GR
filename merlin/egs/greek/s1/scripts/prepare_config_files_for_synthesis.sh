...
echo "train_file_number: 754" >> $duration_config_file
...
echo "in_shape_dir: %(data)s/shape" >> $acoustic_config_file
echo "in_texture_dir: %(data)s/texture" >> $acoustic_config_file
...
echo "sptk:  ${SPTK}" >> $acoustic_config_file
...
echo "aam_model: ${aam_model}" >> $acoustic_config_file

if [ "$Vocoder" == "STRAIGHT" ] 
then
    echo "STRAIGHT_DIR: $STRAIGHT_DIR" >> $acoustic_config_file
    # echo "sp_dim: 1025" >> $acoustic_config_file
...
then
    ...
    echo "shape    : 9" >> $acoustic_config_file
    echo "dshape    : 27" >> $acoustic_config_file
fi
...
echo "" >> $acoustic_config_file
echo "test_synth_dir :  %(TOPLEVEL)s/experiments/${Voice}/${test_synth_dir}" >> $acoustic_config_file  
